...
module.exports = {
  ...
  init: (config) => {
    ...
    return {
      upload: (file) => {
       log('prepare to upload file: %j', {...file, buffer: 'hide me'});
        return new Promise((resolve, reject) => {
              ...
              if (!err) {
                ...
                ossClient.putStream(`${path}${file.hash}-${THUMBNAIL_SIZE}.png`, proc.stdout)
                  .then((result) => {
                    ...
                    log('thumbnail generated ok: %j', result);
                  })
                  .catch((err) => {
                    log('[error] thumbnail generation failed: %j', err);
                  });

              } else {
                log('[error] ffmpeg not found, therefore no thumbnails are generated ');
              }
            })
          };
          ...
          ossClient.put(fullPath, fileBuffer)
            .then((result) => {
              log("upload file to oss result: %j", {result, buffer: 'hide me'});
              if (config.baseUrl) {
      ...
      delete: (file) => {
        return new Promise((resolve, reject) => {
          ...
          ossClient.delete(fullPath)
            .then((resp) => {
              log('delete file on OSS resp: %j', resp);
